...
const mockLocalStorage = (() => {
  ...
  describe('useStorage', () => {
    ...
    test('initializes movements from localStorage', () => {
        const mockData = [{ id: 1, title: 'Test Movement', amount: 100, description: 'Test description' }];
    ...
    test('adds a movement', () => {
        ...
        expect(localStorage.getItem('movements')).toContain(JSON.stringify(newMovement));
    })

    test('deletes a movement', () => {
        const existingMovement: Movement = { id: 3, title: 'Existing Movement', amount: 300, description: 'definition 3', time: new Date() };
        addMovement(existingMovement);
        deleteMovement(existingMovement.id);
    
        expect(movements.value).not.toContainEqual(existingMovement);
        expect(localStorage.getItem('movements')).not.toContain(JSON.stringify(existingMovement));
      });
  })